#!/usr/bin/env bash
# /qompassai/Arch/omarchy/4200-nvidia/install/first-run/modules-load.sh
...
if [[ $HAS_NVIDIA -eq 1 ]]; then
    cat > "$MLOAD_DIR/nvidia.conf" << 'EOF'
nvidia
nvidia_drm
nvidia_modeset
nvidia_uvm
EOF
    sudo chmod 644 "$MLOAD_DIR/nvidia.conf"
fi
...
if [[ $HAS_INTEL -eq 1 ]]; then
    ...
    } > "$MLOAD_DIR/intel.conf"
    sudo chmod 644 "$MLOAD_DIR/intel.conf"
fi
